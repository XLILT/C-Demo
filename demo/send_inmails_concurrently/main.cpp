...
static int MAX_SEND_COUNTS_PER_REQUEST = 1000;
...
int main(int argc, char **argv)
{
    ...
    do
    {
        ...
        MAX_SEND_COUNTS_PER_REQUEST = argc >= 2 ? atoi(argv[2]) : MAX_SEND_COUNTS_PER_REQUEST;
        for (int i = 0; i < thread_cnt; ++i)
        {
            th_group.add_thread(new boost::thread(send_mails, sub, body, uid_vec));
        }
